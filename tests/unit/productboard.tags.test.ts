import {
  ...
  createTag,
  ensureTagsExist,
  TagProvisioningUnavailableError,
...
describe('createTag', () => {
  it('throws TagProvisioningUnavailableError (PB endpoint returns 500 — see memory)', async () => {
    // No fetch mock — createTag should throw without making any HTTP call.
    global.fetch = jest.fn() as unknown as typeof fetch;
    await expect(createTag('any-name')).rejects.toBeInstanceOf(TagProvisioningUnavailableError);
...
describe('ensureTagsExist', () => {
  ...
  it('drops unknown names with a logged warning rather than failing the call', async () => {
    ...
    expect(out.has('unknown-tag-name')).toBe(false);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('unknown-tag-name'));
    warn.mockRestore();
  });
